...
RF=2 replication doubles storage and I/O costs without providing AZ-level disaster recovery. We designed a **dual-cluster architecture** — two independent Victoria Metrics clusters (VM-A, VM-B) in separate AZ, each holding 100% of data at RF=1. Dual-write at vmagent layer ensures zero-gap ingestion. Read path isolation (via Kubernetes `topology-mode: Auto` and NLB zonal affinities) eliminates cross-AZ traffic on queries, reducing egress costs by ~$1.7K/month while improving availability.

**Key results:**
- Eliminated vmselect fan-out across 20 vmstorage nodes in 3 AZ → **10 nodes per cluster (−50% memory pressure)**
- Cross-AZ read traffic: **3 hops → 0**
- Write I/O per cluster: **×2 (RF=2) → ×1**
- DR: RPO=0 at AZ level with independent failure domains
...
**Write path:**
- Internal scrapers: kube-proxy topology fallback → buffer-b (automatic, <100ms)
- External scrapers: NLB health check (~20s) → removes 1a targets → routes to buffer-b
- Both buffer-b pods continue dual-write; VM-A writes queue on disk

**Read path:**
- Grafana pods: kube-proxy topology fallback → vmselect-b (automatic, <100ms)
- External tools: NLB health check (~20s) → routes to vmselect-b
- All data already replicated to VM-B, queries succeed

**Recovery (1a comes back):**
- Health checks pass → NLB re-adds 1a targets
- Pending disk buffers drain automatically (rate-limited)
- ~5 minute to full capacity
...
|-----------|---|---|---|
| Write I/O per cluster | 2× samples/sec | 1× | −$400 |
| vmselect pods (fan-out) | 20 nodes | 10 nodes | −$280 |
| Cross-AZ egress (read) | ~$900 | ~$0 | −$900 |
| Dual-write overhead | — | ~$950 | +$950 |
| **Net savings** | | | **~$1,630/month** |

One-time investment: 3 weeks design + 2 weeks implementation + 1 week stage validation = ~160 engineer hours.
